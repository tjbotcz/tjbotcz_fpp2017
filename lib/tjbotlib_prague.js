 ...
 */
TJBot.prototype.converse = function(workspaceId, message, context, callback) {
...
TJBot.prototype.listen = function(callback) {
    ...
    if(this.configuration.listen.customization_id){
      winston.info("Customization_id is set.");
      ...
      winston.info("Customization_id is not set.");
      this._micRecognizeStream = this._stt.createRecognizeStream({
          content_type: 'audio/l16; rate=44100; channels=2',
          inactivity_timeout: this.configuration.listen.inactivityTimeout,
          model: this.configuration.listen.language + "_BroadbandModel"
...
/*
[newly in PRAGUE TJBOT VERSION]
Recognize faces in photo by sending it to the Watson Visual Recognition Service.
 */
TJBot.prototype.detectFacesInPhoto = function(filePath) {
    this._assertCapability('see');
    ...
    return new Promise(function(resolve, reject) {
        winston.debug("sending image to Watson Visual Recognition to detect faces.");
        var params = {
            images_file: fs.createReadStream(filePath)
        };
...
/**
 * Turn off all colorsChange the color of the RGB led.
 *
*/
